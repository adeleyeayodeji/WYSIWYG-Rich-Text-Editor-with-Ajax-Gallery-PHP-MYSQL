...
function insertIMG2(src, id, name, date) {
    $(".note-image-url").val(src);
    $("#clkbtn").removeAttr("disabled");

    $("#getpic22").attr("src", src);

    var all2 = document.getElementsByClassName('gallerypic2');
    for (var i = 0; i < all2.length; i++) {
        all2[i].style.background = 'white';
    }
    document.getElementById(id).style.background = "#3555ff";
    // console.log(id);

    $(document).ready(function() {
        $("#detailimg2").html(`
    <h3 class="preview_header">Attachment Details</h3>
    <img src="${src}" class="preview_img_me">
    <div class="filename_preview">${name}</div>
    <p class="mb-0" style="font-size: 11px;
    color: gray;">${new Date(date)}</p>
    <a href="javascript:;" class="text-danger deletebtn" onclick="deleteIMG2('${name}', '${id}')">Delete Permanently</a>
    <input type="text" class="form-control" value="${src}" style="font-size: 11px;margin-top: 5px;" readonly/>
    `).hide().fadeIn();
    });

    $("#button").removeAttr("disabled").css("box-shadow", "2px 2px 2px 2px black").hide().fadeIn();
}

function deleteIMG2(name, id) {
    $(document).ready(function() {
        $.ajax({
            type: "POST",
            url: API_URL + "include/deleteImg.php",
            data: {
                "delete": 1,
                "name": name
            },
            success: function(response) {
                if (response.info == "Image deleted") {
                    var img = document.getElementById(id);
                    // img.style.display = "none";
                    $(img).fadeOut();
                    document.getElementById("detailimg2").innerHTML = ""
                        ...
                        _createClass(Context, [{
                            ...
                            value: function createInvokeHandler(namespace, value) {
                                ...
                                return function(event) {
                                    ...
                                    //Condition
                                    var images = document.getElementsByClassName("gallerypic2");
                                    if (images.length < 1) {
                                        //Load gallery on pop up
                                        $.ajax({
                                            type: "GET",
                                            url: API_URL + "include/gallery.php",
                                            beforeSend: function() {
                                                $("#ajazresponse2").html(`
                                                <div id="currentpicm">
                                                   <center><img class="gallerypic2" src="img/loading.gif"></center>
                                                </div>
                                                `);
                                            },
                                            success: function(response) {
                                                $("#currentpicm").remove();
                                                // console.log("Feature mi ", response);
                                                for (let index = 0; index < response.length; index++) {
                                                    const result = `
                                                <div>
                                                <img class="gallerypic2 lazy" src="img/loading.gif" id="${response[index].imagesrc}featured" onclick="insertIMG2(this.src, this.id, '${response[index].name}','${response[index].date_uplaoded}')" data-src="${API_URL}image/${response[index].imagesrc}" data-srcset="${API_URL}image/${response[index].imagesrc}, ${API_URL}image/${response[index].imagesrc}" />
                                                </div>
                                                `;
                        ...
                        ImageDialog_createClass(ImageDialog, [{
                            ...
                            value: function initialize() {
                                ...
                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                  <a class="nav-item nav-link active" id="recent" data-toggle="tab" href="#recent-tab" role="tab" aria-controls="recent-tab" aria-selected="true">Recent</a>
                                  <a class="nav-item nav-link" id="upload" data-toggle="tab" href="#upload-tab" role="tab" aria-controls="upload-tab" aria-selected="false">Upload</a>
                                </div>
                                </nav>
                                <div class="tab-content" id="nav-tabContent" style="height: 335px;overflow: scroll;">
                                  <div class="tab-pane fade show active" id="recent-tab" role="tabpanel" aria-labelledby="recent">
                                    <div class="row">
                                    <div class="col-9">
                                      <div class="row ml-0 mr-0">
                                        <div id="ajazresponse2" class="col" style="display:contents;">
                                        
                                        </div>
                                      </div>
                                    </div>
                                    <div id="detailimg2" class="col pl-0" style="overflow:scroll;">
                                      
                                    </div>
                                    </div>
                                  </div>
                                  <div class="tab-pane fade" id="upload-tab" role="tabpanel" aria-labelledby="upload">
                                  <label style="width: 100%;
                                  height: 100%;">
                                  <input type="file" multiple="multiple" name="userImage" style="display: none;" id="uploadimage_src" onchange="imagepreview_upload(event)">
                                  <div class="row ml-0 mr-0">
                                    <div id="ajazresponse3" class="col" style="display:contents;">
                                    </div>
                                    <div class="col" id="clickme">
                                        ...
                                        margin-bottom: 10px;">
                                            ...
                                            <p>or</p>
                                            <button type="button" onclick="$('#uploadimage_src').trigger('click');" class="btn btn-primary">Browse Files</button>
                                        </div>
                                    </div>
                                  </div>
                                  </label>
                                ...
                                <input style="display:none;" id="note-dialog-image-url-${this.options.id}" class="note-image-url form-control note-form-control note-input" type="text"/>
      `;
                                var buttonClass = 'btn btn-primary note-btn note-btn-primary note-image-btn';
                                var footer = "<input id=\"clkbtn\" type=\"button\" href=\"#\" class=\"".concat(buttonClass, "\" value=\"").concat("insert into post", "\" disabled>");
                                this.$dialog = this.ui.dialog({
            ...
            /***/
                ...
                var dialog = renderer["a" /* default */ ].create('<div class="modal note-modal" aria-hidden="false" tabindex="-1" role="dialog"/>', function($node, options) {
                    ...
                    $node.html(['<div class="modal-dialog modal-xl">', '<div class="modal-content">', options.title ? '<div class="modal-header">' + '<h4 class="modal-title">' + options.title + '</h4>' + '<button type="button" class="close" data-dismiss="modal" aria-label="Close" aria-hidden="true">&times;</button>' + '</div>' : '', '<div class="modal-body">' + options.body + '</div>', options.footer ? '<div class="modal-footer">' + options.footer + '</div>' : '', '</div>', '</div>'].join(''));
                });
